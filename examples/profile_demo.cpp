...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::cerr << "Usage: profile_demo kv_port profiles.tsv source_revision\n";
        return 1;
    }
    ...
    try {
        ...
        std::atomic<uint64_t> source_reads{0};
        ...
        service.Register<std::string, std::string>("profile.get", [&](const std::string& id) {
            ...
            while (std::getline(file, line)) {
                ...
                try {
                    cache.Set(key(id), profile).get();
                } catch (const kvrpc::Error&) {
                }
                return profile;
            }
            throw std::runtime_error("Profile not found");
        });
        service.Register<void, std::string>("profile.invalidate",
                                            [&](const std::string& id) { cache.Delete(key(id)).get(); });
        service.Register<uint64_t>("profile.source_reads", [&] { return source_reads.load(); });
