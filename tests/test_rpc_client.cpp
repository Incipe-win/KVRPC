...
void run_mock_server() {
    ...
    std::cout << "[Mock Server] Received RPC call: " << method << " with args: " << payload_str << std::endl;
...
int main() {
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(100)); // 保证服务器先启动
    ...
    std::future<std::string> fut = client.Call<std::string>("ECHO", std::string("hello async rpc"));

    // 主线程此时可以做别的事...
    std::cout << "[Client] Doing other works while waiting for I/O..." << std::endl;
