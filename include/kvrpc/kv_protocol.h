...
inline constexpr size_t MAX_FRAME_SIZE = HEADER_SIZE + MAX_KEY_SIZE + MAX_VALUE_SIZE;
...
struct Message {
    ...
    static void validate(const Header& h) {
        if (h.magic != MAGIC || h.version != VERSION || h.command < 1 || h.command > 4 ||
            h.key_len > MAX_KEY_SIZE || h.value_len > MAX_VALUE_SIZE)
    ...
    static std::vector<uint8_t> encode(Command command, const std::string& key, const std::string& value = "") {
        ...
        bytes[2] = VERSION; bytes[3] = header.command;
    ...
    static Header decodeHeader(const uint8_t* bytes) {
        ...
        h.magic = ntohs(h.magic); h.version = bytes[2]; h.command = bytes[3];
        std::memcpy(&h.key_len, bytes + 4, 4); h.key_len = ntohl(h.key_len);
        std::memcpy(&h.value_len, bytes + 8, 4); h.value_len = ntohl(h.value_len);
        return h;
    }
};
}  // namespace kvcache
