...
public:
    explicit Executor(ClientOptions options = {}) : capacity_(options.queue_capacity) {
        if (!options.workers || options.workers > 256 || !capacity_ || capacity_ > 65536 ||
            !options.max_frame_bytes || options.max_frame_bytes > 64 * 1024 * 1024)
    ...
    template<class F> auto Submit(F&& function) -> std::future<std::invoke_result_t<F>> {
        ...
        auto result = task->get_future();
        {
            ...
            if (stopping_ || tasks_.size() >= capacity_) {
                ...
                rejected.set_exception(std::make_exception_ptr(Error(
                    stopping_ ? ErrorCode::closed : ErrorCode::overloaded, "Client queue is closed or full")));
