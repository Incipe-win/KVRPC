...
public:
    ConnectionPool(const std::string& ip, uint16_t port, size_t pool_size)
        ...
        // Here we initialize them lazy or proactively. Let's do lazy loading inside Acquire.
        // But for pool budgeting, we just push `pool_size` empty connections.
    ...
    std::shared_ptr<TcpConnection> Acquire() {
        ...
        return std::shared_ptr<TcpConnection>(conn, [this](TcpConnection* p) {
            this->Release(p);
        });
    }
